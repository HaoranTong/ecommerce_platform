...
# 使用本地测试配置
from conftest_inventory import (
    db_session, test_client, admin_user, normal_user, 
    test_sku_inventory, multiple_test_inventories
)
...
class TestInventoryCore:
    ...
    def test_create_sku_inventory(self, db_session: Session):
        """测试创建SKU库存 - 验证架构合规性"""
        ...
        # 执行创建
        result = asyncio.run(service.create_sku_inventory(inventory_data))
        
        # 验证结果符合架构要求
        assert result.sku_id == "TEST_SKU_001"  # 使用sku_id而不是product_id
    ...
    def test_get_sku_inventory(self, db_session: Session, test_sku_inventory: InventoryStock):
        """测试获取SKU库存 - 验证架构合规性"""
        service = InventoryService(db_session)
        
        # 获取库存
        result = asyncio.run(service.get_sku_inventory("SKU001001"))
        
        # 验证返回数据符合架构
        assert result is not None
        assert result.sku_id == "SKU001001"  # 确认使用sku_id
        assert result.total_quantity == 100
        assert result.available_quantity == 100
        assert result.reserved_quantity == 0

    def test_batch_inventory_query(self, db_session: Session, multiple_test_inventories: List[InventoryStock]):
        """测试批量库存查询 - 验证架构合规性"""
        service = InventoryService(db_session)
        
        # 批量查询
        sku_ids = ["SKU002001", "SKU002002", "SKU002003"]
        result = asyncio.run(service.get_batch_inventory(sku_ids))
    ...
    def test_inventory_reservation_flow(self, db_session: Session, test_sku_inventory: InventoryStock):
        """测试库存预占流程 - 完整业务流程验证"""
        service = InventoryService(db_session)
        
        # 1. 预占库存
        items = [ReservationItem(sku_id="SKU001001", quantity=30)]
        ...
        # 4. 验证库存恢复
        final_inventory = asyncio.run(service.get_sku_inventory("SKU001001"))
        assert final_inventory.available_quantity == 100
        assert final_inventory.reserved_quantity == 0

    def test_inventory_deduction_flow(self, db_session: Session, test_sku_inventory: InventoryStock):
        """测试库存扣减流程 - 验证订单出库"""
        service = InventoryService(db_session)
        
        # 1. 先预占
        items = [ReservationItem(sku_id="SKU001001", quantity=25)]
        reservation = asyncio.run(service.reserve_inventory(
        ...
        assert deduct_result.deducted_items[0].sku_id == "SKU001001"
    ...
    def test_inventory_adjustment_operations(self, db_session: Session, test_sku_inventory: InventoryStock):
        """测试库存调整操作 - 验证管理员功能"""
        service = InventoryService(db_session)
        ...
        assert increase_result.sku_id == "SKU001001"
        ...
        assert decrease_result.sku_id == "SKU001001"
        assert decrease_result.old_quantity == 150
        assert decrease_result.new_quantity == 120

    def test_low_stock_warning_system(self, db_session: Session, multiple_test_inventories: List[InventoryStock]):
        """测试库存预警系统"""
        service = InventoryService(db_session)
        
        # 获取预警级别的低库存
        warning_query = LowStockQuery(level="warning", limit=10, offset=0)
        warning_result = asyncio.run(service.get_low_stock_skus(warning_query))
        
        # 应该找到库存为8的SKU（低于warning_threshold=10）
        warning_skus = [item.sku_id for item in warning_result]
        assert "SKU002002" in warning_skus  # 库存8，低于阈值10
        assert "SKU002003" in warning_skus  # 库存3，低于阈值10
        
        # 获取严重不足级别的库存
        critical_query = LowStockQuery(level="critical", limit=10, offset=0)
        critical_result = asyncio.run(service.get_low_stock_skus(critical_query))
        
        # 应该只找到库存为3的SKU（低于critical_threshold=5）
        critical_skus = [item.sku_id for item in critical_result]
        assert "SKU002003" in critical_skus  # 库存3，低于阈值5
        assert "SKU002002" not in critical_skus  # 库存8，高于阈值5

    def test_inventory_transaction_logging(self, db_session: Session, test_sku_inventory: InventoryStock):
        """测试库存变动记录 - 验证审计功能"""
        service = InventoryService(db_session)
        ...
        logs = asyncio.run(service.get_transaction_logs(query))
        
        # 验证记录存在
        assert logs.sku_id == "SKU001001"
        assert logs.total >= 1
        assert len(logs.logs) >= 1
        
        # 验证记录内容
        latest_log = logs.logs[0]
        assert latest_log.sku_id == "SKU001001"
        assert latest_log.transaction_type == TransactionType.ADJUST
        assert latest_log.quantity_change == 20
        assert latest_log.operator_id == 1001

    def test_data_consistency_validation(self, db_session: Session):
        """测试数据一致性验证"""
        service = InventoryService(db_session)
        
        # 创建一个数据不一致的库存记录
        inconsistent_inventory = InventoryStock(
            sku_id="INCONSISTENT_SKU",
            total_quantity=50,
        ...
        # 执行一致性检查
        check_result = asyncio.run(service.check_inventory_consistency())
        ...
        inconsistent_item = next(
            (item for item in check_result.details if item.sku_id == "INCONSISTENT_SKU"),
            None
        )
        assert inconsistent_item is not None
        assert "不等于" in inconsistent_item.issue or "大于" in inconsistent_item.issue

    def test_expired_reservation_cleanup(self, db_session: Session, test_sku_inventory: InventoryStock):
        """测试过期预占清理"""
        service = InventoryService(db_session)
        ...
        inventory = db_session.query(InventoryStock).filter(
            InventoryStock.sku_id == "SKU001001"
        ).first()
        ...
        # 执行清理
        cleanup_result = asyncio.run(service.cleanup_expired_reservations())
        ...
        # 验证库存已恢复
        updated_inventory = asyncio.run(service.get_sku_inventory("SKU001001"))
        assert updated_inventory.available_quantity == 100
        assert updated_inventory.reserved_quantity == 0

    def test_error_handling_and_boundaries(self, db_session: Session, test_sku_inventory: InventoryStock):
        """测试错误处理和边界情况"""
        service = InventoryService(db_session)
        
        # 测试预占数量超过可用库存
        items = [ReservationItem(sku_id="SKU001001", quantity=150)]  # 超过100的可用库存
        ...
        # 测试不存在的SKU操作
        result = asyncio.run(service.get_sku_inventory("NONEXISTENT_SKU"))
        assert result is None
        
        # 测试重复创建同一SKU的库存
        duplicate_data = SKUInventoryCreate(
            sku_id="SKU001001",  # 已存在
            initial_quantity=50,
        ...
        with pytest.raises(ValueError, match="已存在"):
            asyncio.run(service.create_sku_inventory(duplicate_data))
