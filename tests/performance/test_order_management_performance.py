...
# Target path: tests/performance/test_order_management_performance.py
...
class TestOrderManagementResponseTime:
    """订单管理模块响应时间性能测试"""
    ...
    async def test_api_response_time_p50(self, async_api_client):
        ...
        for _ in range(warmup_count):
            await async_api_client.get("/api/v1/order-management/orders", headers=headers)
        ...
        for _ in range(100):
            ...
            response = await async_api_client.get("/api/v1/order-management/orders", headers=headers)
    ...
    async def test_database_query_performance(self, async_api_client):
        ...
        # 测试不同类型的查询性能
        query_endpoints = [
            f"/api/v1/order-management/",           # 列表查询
            f"/api/v1/order-management/search",     # 搜索查询
            f"/api/v1/order-management/1",          # 单记录查询
        ]
...
from tests.factories import product_catalog_factories
from tests.factories import inventory_management_factories

class TestOrderManagementConcurrency:
    """订单管理模块并发性能测试"""
    ...
    async def test_concurrent_write_requests(self, async_api_client):
        ...
        # 🔧 为每个SKU创建InventoryStock记录（订单创建需要验证SKU库存）
        for factory_class in inventory_management_factories.__dict__.values():
            if hasattr(factory_class, '_meta') and hasattr(factory_class._meta, 'sqlalchemy_session'):
                factory_class._meta.sqlalchemy_session = async_api_client.db
        for sku in test_sku_fixtures:
            inventory_management_factories.InventoryStockFactory.create(sku_id=sku.id, total_quantity=1000, available_quantity=1000)
        async_api_client.db.flush()  # 确保InventoryStock数据提交
        ...
        async def request_operation(request_id):
            ...
            try:
                ...
                if "POST" == "GET":
                    ...
                    fake = Faker()
                    test_data = {
                        "items": [{"product_id": random.choice(test_product_fixtures).id, "sku_id": random.choice(test_sku_fixtures).id, "quantity": fake.random_int(min=1, max=999)}],
        ...
        async def write_operation():
            ...
            try:
                ...
                if "POST" == "GET":
                    ...
                    response = await async_api_client.put("/api/v1/order-management/orders", json=test_data, headers=headers)
                else:
                    # POST请求
                    test_data = {
                        "items": [{"product_id": random.choice(test_product_fixtures).id, "sku_id": random.choice(test_sku_fixtures).id, "quantity": fake.random_int(min=1, max=999)}],
                ...
                "notes": fake.text(max_nb_chars=50)
                    ...
                    response = await async_api_client.post("/api/v1/order-management/orders", json=test_data, headers=headers)
...
class TestOrderManagementLoadTest:
    """订单管理模块负载测试"""
    ...
    async def test_sustained_load(self, async_api_client):
        ...
        while time.time() - start_time < duration_seconds:
            ...
            for _ in range(requests_per_second):
                task = async_api_client.get("/api/v1/order-management/orders", headers=headers)
    ...
    async def test_peak_load_handling(self, async_api_client):
        ...
        async def user_session():
            ...
            try:
                # 用户典型操作序列（只使用读操作）
                operations = [
                    ("GET", "/api/v1/order-management/orders"),  # 获取当前用户信息
                    ("GET", "/api/v1/order-management/"),  # 列表
                    ("GET", "/api/v1/order-management/orders"),  # 再次获取用户信息
                ]
...
class TestOrderManagementBenchmark:
    """订单管理模块性能基准测试"""
    ...
    async def test_performance_regression(self, async_api_client):
        ...
        for _ in range(30):
            start = time.time()
            response = await async_api_client.get(f"/api/v1/order-management/search", params={"q": "test"}, headers=headers)
            end = time.time()
        ...
        throughput_tasks = [async_api_client.get("/api/v1/order-management/orders", headers=headers) for _ in range(100)]
