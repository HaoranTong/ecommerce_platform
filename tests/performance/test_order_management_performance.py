...
class TestOrderManagementConcurrency:
    ...
    async def test_concurrent_write_requests(self, async_api_client):
        ...
        # 获取数据库session
        db_generator = mysql_integration_db()
        db = next(db_generator)
        
        # 创建产品测试数据
        test_product_fixtures = [StandardTestDataFactory.create_complete_chain(db)[3] for _ in range(5)]  # 创建5个产品
        test_sku_fixtures = [StandardTestDataFactory.create_complete_chain(db)[4] for _ in range(5)]  # 创建5个SKU
