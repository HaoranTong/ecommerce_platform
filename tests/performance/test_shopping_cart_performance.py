...
# Generated at: 20251014_171052
# Target path: tests/performance/test_shopping_cart_performance.py
...
class TestShoppingCartResponseTime:
    """购物车模块响应时间性能测试"""
    ...
    async def test_api_response_time_p50(self, async_api_client):
        ...
        for _ in range(warmup_count):
            await async_api_client.get("/api/v1/shopping-cart/cart", headers=headers)
    ...
    async def test_database_query_performance(self, async_api_client):
        ...
        # 测试不同类型的查询性能
        query_endpoints = [
            f"/api/v1/shopping-cart/",           # 列表查询
            f"/api/v1/shopping-cart/search",     # 搜索查询
            f"/api/v1/shopping-cart/1",          # 单记录查询
        ]
...
class TestShoppingCartConcurrency:
    ...
    async def test_concurrent_write_requests(self, async_api_client):
        ...
        test_sku_fixtures = product_catalog_factories.ProductFactory.create_batch(100)
        ...
        async def request_operation(request_id):
            ...
            try:
                ...
                if "POST" == "GET":
                    ...
                    fake = Faker()
                    test_data = {
                        "sku_id": random.choice(test_sku_fixtures).id,
    ...
    async def test_mixed_workload_performance(self, async_api_client):
        ...
        async def write_operation():
            ...
            try:
                # 根据HTTP方法决定是否需要数据
                if "POST" == "GET":
                    response = await async_api_client.get("/api/v1/shopping-cart/items", headers=headers)
                elif "POST" == "PUT":
                    # PUT请求使用简单的更新数据
                    test_data = {"real_name": f"Test User {fake.random_int(min=1000, max=9999)}"}
                    response = await async_api_client.put("/api/v1/shopping-cart/items", json=test_data, headers=headers)
                else:
                    # POST请求
                    test_data = {
                        "sku_id": random.choice(test_sku_fixtures).id,
                "quantity": fake.random_int(min=1, max=999)
                    ...
                    response = await async_api_client.post("/api/v1/shopping-cart/items", json=test_data, headers=headers)
...
class TestShoppingCartLoadTest:
    """购物车模块负载测试"""
    ...
    async def test_sustained_load(self, async_api_client):
        ...
        while time.time() - start_time < duration_seconds:
            ...
            for _ in range(requests_per_second):
                task = async_api_client.get("/api/v1/shopping-cart/cart", headers=headers)
    ...
    async def test_peak_load_handling(self, async_api_client):
        ...
        async def user_session():
            ...
            try:
                # 用户典型操作序列（只使用读操作）
                operations = [
                    ("GET", "/api/v1/shopping-cart/cart"),  # 获取当前用户信息
                    ("GET", "/api/v1/shopping-cart/"),  # 列表
                    ("GET", "/api/v1/shopping-cart/cart"),  # 再次获取用户信息
                ]
...
class TestShoppingCartBenchmark:
    """购物车模块性能基准测试"""
    ...
    async def test_performance_regression(self, async_api_client):
        ...
        for _ in range(30):
            start = time.time()
            response = await async_api_client.get(f"/api/v1/shopping-cart/search", params={"q": "test"}, headers=headers)
            end = time.time()
        ...
        throughput_tasks = [async_api_client.get("/api/v1/shopping-cart/cart", headers=headers) for _ in range(100)]
