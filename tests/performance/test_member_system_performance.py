...
# Target path: tests/performance/test_member_system_performance.py
...
class TestMemberSystemResponseTime:
    """会员系统模块响应时间性能测试"""
    ...
    async def test_api_response_time_p50(self, async_api_client):
        ...
        for _ in range(warmup_count):
            await async_api_client.get("/api/v1/profile", headers=headers)
    ...
    async def test_database_query_performance(self, async_api_client):
        ...
        # 测试不同类型的查询性能
        query_endpoints = [
            f"/api/v1/member-system/",           # 列表查询
            f"/api/v1/member-system/search",     # 搜索查询
            f"/api/v1/member-system/1",          # 单记录查询
        ]
...
class TestMemberSystemConcurrency:
    """会员系统模块并发性能测试"""
...
class TestMemberSystemLoadTest:
    """会员系统模块负载测试"""
    ...
    async def test_sustained_load(self, async_api_client):
        ...
        while time.time() - start_time < duration_seconds:
            ...
            for _ in range(requests_per_second):
                task = async_api_client.get("/api/v1/profile", headers=headers)
        ...
        async def user_session():
            ...
            try:
                ...
                operations = [
                    ...
                    ("GET", "/api/v1/member-system/"),  # 列表
                    ("GET", "/api/v1/profile"),  # 再次获取用户信息
                ]
...
class TestMemberSystemBenchmark:
    """会员系统模块性能基准测试"""
    ...
    async def test_performance_regression(self, async_api_client):
        ...
        for _ in range(30):
            start = time.time()
            response = await async_api_client.get(f"/api/v1/member-system/search", params={"q": "test"}, headers=headers)
            end = time.time()
        ...
        throughput_tasks = [async_api_client.get("/api/v1/profile", headers=headers) for _ in range(100)]
