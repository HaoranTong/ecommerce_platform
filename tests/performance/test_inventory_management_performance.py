...
# Target path: tests/performance/test_inventory_management_performance.py
...
class TestInventoryManagementResponseTime:
    """库存管理模块响应时间性能测试"""
    ...
    async def test_api_response_time_p50(self, async_api_client):
        ...
        for _ in range(warmup_count):
            await async_api_client.get("/api/v1/inventory-management/stock/{sku_id}", headers=headers)
    ...
    async def test_database_query_performance(self, async_api_client):
        ...
        # 测试不同类型的查询性能
        query_endpoints = [
            f"/api/v1/inventory-management/",           # 列表查询
            f"/api/v1/inventory-management/search",     # 搜索查询
            f"/api/v1/inventory-management/1",          # 单记录查询
        ]
...
class TestInventoryManagementConcurrency:
    """库存管理模块并发性能测试"""
    ...
    async def test_mixed_workload_performance(self, async_api_client):
        ...
        async def write_operation():
            ...
            try:
                # 根据HTTP方法决定是否需要数据
                if "POST" == "GET":
                    response = await async_api_client.get("/api/v1/inventory-management/stock/batch", headers=headers)
                elif "POST" == "PUT":
                    # PUT请求使用简单的更新数据
                    test_data = {"real_name": f"Test User {fake.random_int(min=1000, max=9999)}"}
                    response = await async_api_client.put("/api/v1/inventory-management/stock/batch", json=test_data, headers=headers)
                else:
                    # POST请求
                    test_data = {
                        "warning_threshold": fake.random_int(min=0, max=999999),
                "critical_threshold": fake.random_int(min=0, max=999999)
                    ...
                    response = await async_api_client.post("/api/v1/inventory-management/stock/batch", json=test_data, headers=headers)
...
class TestInventoryManagementLoadTest:
    """库存管理模块负载测试"""
    ...
    async def test_sustained_load(self, async_api_client):
        ...
        while time.time() - start_time < duration_seconds:
            ...
            for _ in range(requests_per_second):
                task = async_api_client.get("/api/v1/inventory-management/stock/{sku_id}", headers=headers)
    ...
    async def test_peak_load_handling(self, async_api_client):
        ...
        async def user_session():
            ...
            try:
                # 用户典型操作序列（只使用读操作）
                operations = [
                    ("GET", "/api/v1/inventory-management/stock/{sku_id}"),  # 获取当前用户信息
                    ("GET", "/api/v1/inventory-management/"),  # 列表
                    ("GET", "/api/v1/inventory-management/stock/{sku_id}"),  # 再次获取用户信息
                ]
...
class TestInventoryManagementBenchmark:
    """库存管理模块性能基准测试"""
    ...
    async def test_performance_regression(self, async_api_client):
        ...
        for _ in range(30):
            start = time.time()
            response = await async_api_client.get(f"/api/v1/inventory-management/search", params={"q": "test"}, headers=headers)
            end = time.time()
        ...
        throughput_tasks = [async_api_client.get("/api/v1/inventory-management/stock/{sku_id}", headers=headers) for _ in range(100)]
