...
class TestUserAuthE2EWorkflow:
    ...
    async def test_concurrent_user_workflow(self, async_api_client):
        ...
        async def single_user_workflow(user_id):
            ...
            # 执行简单的CRUD操作
            response = await async_api_client.get(f"/api/v1/user-auth/", headers=headers)
...
@pytest.mark.e2e
@pytest.mark.asyncio
class TestUserAuthE2EErrorHandling:
    """
    用户认证模块错误处理E2E测试
    ...
    async def test_unauthorized_access_handling(self, async_api_client):
        ...
        # 不提供认证token
        response = await async_api_client.get(f"/api/v1/user-auth/")
