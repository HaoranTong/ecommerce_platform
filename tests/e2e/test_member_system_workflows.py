...
# Target path: tests/e2e/test_member_system_workflows.py
...
@pytest.mark.e2e
@pytest.mark.asyncio
class TestMemberSystemE2EWorkflow:
    """
    会员系统模块端到端测试
    ...
    async def test_concurrent_user_workflow(self, async_api_client):
        ...
        async def single_user_workflow(user_id):
            ...
            # 执行简单的CRUD操作
            response = await async_api_client.get(f"/api/v1/member-system/", headers=headers)
...
@pytest.mark.e2e
@pytest.mark.asyncio
class TestMemberSystemE2EErrorHandling:
    """
    会员系统模块错误处理E2E测试
    ...
    async def test_unauthorized_access_handling(self, async_api_client):
        ...
        # 不提供认证token
        response = await async_api_client.get(f"/api/v1/member-system/")
