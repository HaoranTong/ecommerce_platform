...
class TestUserAuthPostAPI:
    ...
    "real_name": fake.name()[:50]
        ...
        import subprocess
        redis_key = f"verification_code:register:{email_for_code}"
    ...
    def test_phone_login(self, api_client, mysql_integration_db, mocker):
        ...
        # 步骤2: 从Redis读取验证码（使用docker命令，避免asyncio事件循环冲突）
        import subprocess
        redis_key = f"verification_code:phone_login:{phone_for_code}"
        result = subprocess.run(
            ["docker", "exec", "ecommerce_platform-redis", "redis-cli", "GET", redis_key],
            capture_output=True,
            text=True,
    ...
    def test_reset_password_request(self, api_client, mysql_integration_db, mocker):
        ...
        # 步骤2: 从Redis读取验证码（使用docker命令，避免asyncio事件循环冲突）
        import subprocess
        redis_key = f"verification_code:reset_password:{email_for_code}"
        result = subprocess.run(
            ["docker", "exec", "ecommerce_platform-redis", "redis-cli", "GET", redis_key],
            capture_output=True,
            text=True,
...
class TestUserAuthAPIIntegration:
    ...
    def test_user_auth_workflow(self, api_client):
        """测试user_auth模块完整流程：read -> update -> create -> auth"""
