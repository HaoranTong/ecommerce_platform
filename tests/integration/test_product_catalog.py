...
class TestCategoryModel:
    ...
    def test_create_category_success(self, unit_test_db):
        """测试成功创建分类"""
        category = Category(
            name="电子产品",
            sort_order=1,
    ...
    def test_category_hierarchy(self, unit_test_db):
        """测试分类层次结构"""
        # 创建父分类
        parent = Category(name="数码产品", sort_order=1)
        unit_test_db.add(parent)
        unit_test_db.commit()
        unit_test_db.refresh(parent)
        
        # 创建子分类
        child = Category(name="手机", parent_id=parent.id, sort_order=1)
        unit_test_db.add(child)
        unit_test_db.commit()
        unit_test_db.refresh(child)
        
        assert child.parent_id == parent.id
        assert len(parent.children) == 1
        assert parent.children[0].name == "手机"

    def test_category_soft_delete(self, unit_test_db):
        """测试分类软删除"""
...
class TestBrandModel:
    ...
    def test_create_brand_success(self, unit_test_db):
        """测试成功创建品牌"""
        brand = Brand(
            name="苹果",
            slug="apple",
    ...
    def test_brand_unique_constraints(self, unit_test_db):
        """测试品牌唯一性约束"""
        brand1 = Brand(name="华为", slug="huawei")
        brand2 = Brand(name="华为", slug="huawei-2")
        
        unit_test_db.add(brand1)
        unit_test_db.commit()
        
        # 尝试添加重复名称的品牌
        unit_test_db.add(brand2)
...
class TestProductModel:
    ...
    def test_create_product_success(self, unit_test_db):
        """测试成功创建商品"""
        # 创建品牌和分类
        brand = Brand(name="小米", slug="xiaomi")
        category = Category(name="手机")
        unit_test_db.add_all([brand, category])
        unit_test_db.commit()
        unit_test_db.refresh(brand)
        unit_test_db.refresh(category)
    ...
    def test_product_relationships(self, unit_test_db):
        """测试商品关系"""
        # 创建品牌和分类
    ...
    def test_product_seo_fields(self, unit_test_db):
        """测试商品SEO字段"""
        product = Product(
            name="iPhone 15",
    ...
    def test_product_statistics(self, unit_test_db):
        """测试商品统计字段"""
        product = Product(
            name="测试商品",
            view_count=100,
...
class TestSKUModel:
    ...
    def test_create_sku_success(self, unit_test_db):
        """测试成功创建SKU"""
        # 创建商品
    ...
    def test_sku_unique_code(self, unit_test_db):
        """测试SKU代码唯一性"""
        product = Product(name="测试商品")
        unit_test_db.add(product)
        unit_test_db.commit()
        ...
        unit_test_db.add(sku1)
        unit_test_db.commit()
        
        unit_test_db.add(sku2)
        with pytest.raises(Exception):  # 应该触发唯一约束错误
            unit_test_db.commit()
...
class TestProductAttributeModel:
    ...
    def test_create_product_attribute(self, unit_test_db):
        """测试创建商品属性"""
        product = Product(name="测试商品")
        unit_test_db.add(product)
        unit_test_db.commit()
        ...
        unit_test_db.add(attribute)
        unit_test_db.commit()
        unit_test_db.refresh(attribute)
...
class TestProductBusiness:
    ...
    def test_product_status_filtering(self, unit_test_client, unit_test_db):
        ...
        unit_test_db.add_all([product_draft, product_published])
        unit_test_db.commit()
