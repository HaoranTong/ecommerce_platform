...
class TestInventoryManagement:
    ...
    def test_create_sku_inventory(self, db_session: Session, admin_user: User):
        ...
        assert result.sku_id == "SKU002001"
    ...
    def test_get_sku_inventory(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        """测试获取SKU库存"""
        result = asyncio.run(inventory_service.get_sku_inventory("SKU001001"))
        
        assert result is not None
        assert result.sku_id == "SKU001001"
    ...
    def test_get_batch_inventory(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        """测试批量获取库存"""
        result = asyncio.run(inventory_service.get_batch_inventory(["SKU001001", "SKU999999"]))
        
        assert len(result) == 1
        assert result[0].sku_id == "SKU001001"
    ...
    def test_update_thresholds(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        ...
        # 验证更新结果
        updated = asyncio.run(inventory_service.get_sku_inventory("SKU001001"))
    ...
    def test_reserve_inventory_success(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        ...
        assert result.reserved_items[0].sku_id == "SKU001001"
    ...
    def test_reserve_inventory_insufficient_stock(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        """测试库存预占失败 - 库存不足"""
        items = [ReservationItem(sku_id="SKU001001", quantity=150)]
    ...
    def test_release_reservation(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        """测试释放预占"""
        # 先预占
        items = [ReservationItem(sku_id="SKU001001", quantity=20)]
        ...
        # 验证库存已恢复
        inventory = asyncio.run(inventory_service.get_sku_inventory("SKU001001"))
    ...
    def test_release_user_reservations(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        """测试批量释放用户预占"""
        # 创建多个预占
        items1 = [ReservationItem(sku_id="SKU001001", quantity=15)]
        asyncio.run(inventory_service.reserve_inventory(
            reservation_type=ReservationType.CART,
        ...
        items2 = [ReservationItem(sku_id="SKU001001", quantity=25)]
        asyncio.run(inventory_service.reserve_inventory(
            reservation_type=ReservationType.CART,
            reference_id="user_456",
            items=items2,
    ...
    def test_deduct_inventory_success(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        ...
        assert result.deducted_items[0].sku_id == "SKU001001"
    ...
    def test_get_low_stock_skus(self, inventory_service: InventoryService, db_session: Session):
        ...
        from app.modules.inventory_management.schemas import LowStockQuery
        query = LowStockQuery(level="warning", limit=10, offset=0)
        result = asyncio.run(inventory_service.get_low_stock_skus(query))
        
        assert len(result) >= 1
        assert any(item.sku_id == "SKU003001" for item in result)
    ...
    def test_cleanup_expired_reservations(self, inventory_service: InventoryService, db_session: Session):
        ...
        # 清理过期预占
        result = asyncio.run(inventory_service.cleanup_expired_reservations())
        
        assert result.cleaned_reservations >= 1
        assert result.released_quantity >= 20
    ...
    def test_check_inventory_consistency(self, inventory_service: InventoryService, db_session: Session):
        ...
        # 检查是否发现了不一致问题
        sku_004_issues = [item for item in result.details if item.sku_id == "SKU004001"]
    ...
    def test_api_get_sku_inventory(self, test_sku_inventory: InventoryStock):
        ...
        response = client.get("/api/inventory/stock/SKU001001", headers=headers)
    ...
    def test_api_batch_inventory_query(self, test_sku_inventory: InventoryStock):
        """测试批量查询库存API"""
        headers = {"Authorization": "Bearer test_token"}
        data = {"sku_ids": ["SKU001001", "SKU001002"]}
        
        response = client.post("/api/inventory/stock/batch", json=data, headers=headers)
    ...
    def test_api_reserve_inventory(self, test_sku_inventory: InventoryStock):
        ...
        data = {
            ...
            "reference_id": "user_123",
            "items": [{"sku_id": "SKU001001", "quantity": 10}],
            "expires_minutes": 30
        }
        
        response = client.post("/api/inventory/reserve", json=data, headers=headers)
    ...
    def test_api_deduct_inventory(self, test_sku_inventory: InventoryStock):
        ...
        data = {
            "order_id": "ORD123456",
            "items": [{"sku_id": "SKU001001", "quantity": 5}]
        }
        
        response = client.post("/api/inventory/deduct", json=data, headers=headers)
    ...
    def test_api_adjust_inventory(self, test_sku_inventory: InventoryStock):
        ...
        response = client.post("/api/inventory/adjust/SKU001001", json=data, headers=headers)
    ...
    def test_inventory_boundaries(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        """测试库存边界情况"""
        # 测试0库存预占
        items = [ReservationItem(sku_id="SKU001001", quantity=0)]
    ...
    def test_nonexistent_sku_operations(self, inventory_service: InventoryService):
        """测试不存在SKU的操作"""
        # 获取不存在的SKU
        result = asyncio.run(inventory_service.get_sku_inventory("NONEXISTENT"))
    ...
    def test_concurrent_reservations(self, inventory_service: InventoryService, test_sku_inventory: InventoryStock):
        ...
        # 这里可以添加并发测试逻辑
        # 由于测试环境限制，这里只做基本验证
        items = [ReservationItem(sku_id="SKU001001", quantity=50)]
