...
from app.modules.user_auth.models import (
    Session, Permission, UserRole, RolePermission, Role, User
)
...
class UserAuthFactoryManager:
    ...
    def create_sample_data(session) -> dict:
        ...
        # 2. 创建Session
        data['session'] = SessionFactory(user_id=data['user'].id)
        # 3. 创建Permission
        data['permission'] = PermissionFactory()
        # 4. 创建Role
        data['role'] = RoleFactory()
        # 5. 创建UserRole
        data['userrole'] = UserRoleFactory(user_id=data['user'].id, role_id=data['role'].id)
        # 6. 创建RolePermission
        data['rolepermission'] = RolePermissionFactory(role_id=data['role'].id, permission_id=data['permission'].id)
