...
from app.modules.product_catalog.models import (
    Product, ProductAttribute, Category, ProductImage, SKU, ProductTag, Brand, SKUAttribute
)
...
class ProductCatalogFactoryManager:
    ...
    def create_sample_data(session) -> dict:
        ...
        # 2. 创建Category
        data['category'] = CategoryFactory(parent_id=data['category'].id)
        # 3. 创建Product
        data['product'] = ProductFactory(brand_id=data['brand'].id, category_id=data['category'].id)
        # 4. 创建ProductAttribute
        data['productattribute'] = ProductAttributeFactory(product_id=data['product'].id)
        # 5. 创建SKU
        data['sku'] = SKUFactory(product_id=data['product'].id)
        # 6. 创建ProductImage
        data['productimage'] = ProductImageFactory(product_id=data['product'].id, sku_id=data['sku'].id)
        # 7. 创建ProductTag
        data['producttag'] = ProductTagFactory(product_id=data['product'].id)
        # 8. 创建SKUAttribute
        data['skuattribute'] = SKUAttributeFactory(sku_id=data['sku'].id)
