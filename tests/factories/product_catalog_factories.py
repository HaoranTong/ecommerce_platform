...
from app.modules.product_catalog.models import (
    SKUAttribute, ProductImage, Brand, Category, ProductAttribute, ProductTag, SKU, Product
)
...
class BrandFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    name = factory.LazyFunction(lambda: f'name_{uuid.uuid4().hex[:8]}')
    slug = factory.LazyFunction(lambda: f'slug_{uuid.uuid4().hex[:8]}')
...
class SKUFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    sku_code = factory.LazyFunction(lambda: f'SKU_CODE_{uuid.uuid4().hex[:8].upper()}')
    name = factory.Sequence(lambda n: f'name_{n}')
...
class ProductCatalogFactoryManager:
    ...
    def create_sample_data(session) -> dict:
        ...
        # 3. 创建Product
        data['product'] = ProductFactory(brand=data['brand'], category=data['category'])
        # 4. 创建SKU
        data['sku'] = SKUFactory(product=data['product'])
        # 5. 创建SKUAttribute
        data['skuattribute'] = SKUAttributeFactory(sku=data['sku'])
        # 6. 创建ProductImage
        data['productimage'] = ProductImageFactory(product=data['product'], sku=data['sku'])
        # 7. 创建ProductAttribute
        data['productattribute'] = ProductAttributeFactory(product=data['product'])
        # 8. 创建ProductTag
        data['producttag'] = ProductTagFactory(product=data['product'])
