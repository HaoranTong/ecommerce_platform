...
# Target path: tests/unit/test_models/test_product_catalog_models.py
# Do NOT edit this file manually - regenerate from source models

"""
Product_Catalog 模块数据模型测试
...
class TestBrandModel:
    ...
    def test_is_deleted_field_mock(self, mocker):
        ...
        # 设置字段值
        mock_brand.is_deleted = True
        
        # 验证字段设置
        assert mock_brand.is_deleted == True
        
        # 验证字段类型（如果值不为None）
        if mock_brand.is_deleted is not None:
            expected_type = bool
            assert isinstance(mock_brand.is_deleted, expected_type)
    def test_deleted_at_field_mock(self, mocker):
        ...
        # 设置字段值
        mock_brand.deleted_at = datetime(2025, 1, 1, 12, 0, 0)
        
        # 验证字段设置
        assert mock_brand.deleted_at == datetime(2025, 1, 1, 12, 0, 0)
        
        # 验证字段类型（如果值不为None）
        if mock_brand.deleted_at is not None:
            expected_type = datetime
            assert isinstance(mock_brand.deleted_at, expected_type)
