...
# 被测服务和模型
from app.modules.shopping_cart.models import CartItem, Cart
...
class TestCartService:
    ...
    def test_service_with_mock_cart_repository(self, mocker: MockerFixture):
        """测试Service使用Mock CartRepository
        
        测试策略:
        - Mock CartRepository的方法
        - 验证Service业务逻辑
        - 不依赖数据库
        
        示例: 测试获取Cart的业务逻辑
        """
        print("\n🔧 测试Service Mock CartRepository...")
        ...
        mock_repo = mocker.patch(
            'app.modules.shopping_cart.repository.CartRepository'
        )
        ...
        mock_cart.id = 1
        # 设置其他必要属性
        # mock_cart.name = "Test Cart"
        
        # 设置Mock Repository返回值
        mock_repo.get_by_id.return_value = mock_cart
    ...
    def test_service_with_mock_cartitem_repository(self, mocker: MockerFixture):
        """测试Service使用Mock CartItemRepository
        
        测试策略:
        - Mock CartItemRepository的方法
        - 验证Service业务逻辑
        - 不依赖数据库
        
        示例: 测试获取CartItem的业务逻辑
        """
        print("\n🔧 测试Service Mock CartItemRepository...")
        ...
        # Mock Repository
        mock_repo = mocker.patch(
            'app.modules.shopping_cart.repository.CartItemRepository'
        )
        ...
        mock_cartitem.id = 1
        # 设置其他必要属性
        # mock_cartitem.name = "Test CartItem"
        
        # 设置Mock Repository返回值
        mock_repo.get_by_id.return_value = mock_cartitem
