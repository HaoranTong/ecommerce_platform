...
entity = User(
    ...
    def test_list_query(self, unit_test_db: Session):
        ...
        # 执行Repository方法
        results = UserRepository.list(unit_test_db)  # TODO: 根据实际方法签名调整参数
    ...
    def test_count_query(self, unit_test_db: Session):
        ...
        # 执行Repository方法
        results = UserRepository.count(unit_test_db)  # TODO: 根据实际方法签名调整参数
    ...
    def test_update_single_field(self, unit_test_db: Session):
        ...
        original_password_hash = entity.password_hash
        ...
        assert db_entity.password_hash == original_password_hash
    ...
    def test_increment_failed_login_single_field(self, unit_test_db: Session):
        """测试increment_failed_login - 单字段更新
        ...
        # 执行Repository方法（只更新real_name）
        update_data = {"real_name": "更新后数据"}
        result = UserRepository.increment_failed_login(unit_test_db, entity, update_data)  # TODO: 根据实际方法签名调整
    ...
    def test_increment_failed_login_multiple_fields(self, unit_test_db: Session):
        """测试increment_failed_login - 多字段更新
        ...
        result = UserRepository.increment_failed_login(unit_test_db, entity, update_data)
    ...
    def test_increment_failed_login_transaction_commit(self, unit_test_db: Session):
        """测试increment_failed_login - 事务提交验证
    ...
    def test_increment_failed_login_specialized_method(self, unit_test_db: Session):
        """测试increment_failed_login - 专用方法测试（如有）
...
entity = Role(
    ...
    def test_list_query(self, unit_test_db: Session):
        ...
        # 执行Repository方法
        results = RoleRepository.list(unit_test_db)  # TODO: 根据实际方法签名调整参数
...
entity = Permission(
    ...
    def test_list_query(self, unit_test_db: Session):
        ...
        # 执行Repository方法
        results = PermissionRepository.list(unit_test_db)  # TODO: 根据实际方法签名调整参数
...
entity = UserRole(
    ...
    def test_get_found(self, unit_test_db: Session):
        """测试get - 查询到数据"""
        # 准备测试数据
        entity = UserRole(assigned_at="查询测试")
        unit_test_db.add(entity)
        unit_test_db.commit()
    ...
    def test_get_user_roles_found(self, unit_test_db: Session):
        ...
        entity = UserRole(assigned_at="关联数据")
        unit_test_db.add(entity)
        unit_test_db.commit()
    ...
    def test_delete_success(self, unit_test_db: Session):
        """测试delete - 删除成功"""
        # 准备测试数据
        entity = UserRole(assigned_at="待删除数据")
        unit_test_db.add(entity)
        unit_test_db.commit()
    ...
    def test_delete_all_user_roles_success(self, unit_test_db: Session):
        ...
        # 执行Repository方法（使用主键参数）
        UserRoleRepository.delete_all_user_roles(unit_test_db, user_id_val, role_id_val)
...
entity = RolePermission(
    ...
    def test_get_found(self, unit_test_db: Session):
        """测试get - 查询到数据"""
        # 准备测试数据
        entity = RolePermission(granted_at="查询测试")
        unit_test_db.add(entity)
        unit_test_db.commit()
    ...
    def test_get_role_permissions_found(self, unit_test_db: Session):
        ...
        # 准备关联数据（如UserRole关联User和Role）
        entity = RolePermission(granted_at="关联数据")
        unit_test_db.add(entity)
        unit_test_db.commit()
    ...
    def test_delete_success(self, unit_test_db: Session):
        """测试delete - 删除成功"""
        # 准备测试数据
        entity = RolePermission(granted_at="待删除数据")
        unit_test_db.add(entity)
        unit_test_db.commit()
        role_id_val = entity.role_id
    ...
    def test_delete_all_role_permissions_success(self, unit_test_db: Session):
        ...
        # 执行Repository方法（使用主键参数）
        RolePermissionRepository.delete_all_role_permissions(unit_test_db, role_id_val, permission_id_val)
...
entity = Session(
    ...
    def test_get_by_id_found(self, unit_test_db: Session):
        """测试get_by_id - 查询到数据"""
        # 准备依赖实体
        session = Session(user_id=1, token_hash="Session数据", expires_at="Session数据", last_accessed_at="Session数据", is_active=True)
        unit_test_db.add(session)
        unit_test_db.commit()
        
        # 准备测试数据
        entity = Session(user_id=1, token_hash="查询测试", expires_at="查询测试", last_accessed_at="查询测试", is_active=True)
        unit_test_db.add(entity)
        unit_test_db.commit()
    ...
    def test_get_user_active_sessions_found(self, unit_test_db: Session):
        ...
        # 准备关联数据（如UserRole关联User和Role）
        entity = Session(user_id=1, token_hash="关联数据", expires_at="关联数据", last_accessed_at="关联数据", is_active=True)
        unit_test_db.add(entity)
        unit_test_db.commit()
    ...
    def test_deactivate_single_field(self, unit_test_db: Session):
        """测试deactivate - 单字段更新
        ...
        # 准备测试数据
        from tests.factories.user_auth_factories import SessionFactory
        entity = SessionFactory.create()
        original_token_hash = entity.token_hash
        
        # 执行Repository方法（只更新ip_address）
        update_data = {"ip_address": "更新后数据"}
        result = SessionRepository.deactivate(unit_test_db, entity, update_data)  # TODO: 根据实际方法签名调整
        
        # 验证目标字段已更新
        assert result.ip_address == "更新后数据"
        
        # ✅ 验证其他字段未变化
        assert result.token_hash == original_token_hash
        
        # 验证数据库已更新
        unit_test_db.expire_all()
        db_entity = unit_test_db.query(Session).filter_by(id=entity.id).first()
        assert db_entity.ip_address == "更新后数据"
        assert db_entity.token_hash == original_token_hash
    
    def test_deactivate_multiple_fields(self, unit_test_db: Session):
        """测试deactivate - 多字段更新
        ...
        result = SessionRepository.deactivate(unit_test_db, entity, update_data)
        
        # 验证所有字段已更新
        assert result.ip_address == "更新后数据1"
        assert result.token_hash == "更新后数据2"
        
        # 验证持久化
        unit_test_db.expire_all()
        db_entity = unit_test_db.query(Session).filter_by(id=entity.id).first()
        assert db_entity.ip_address == "更新后数据1"
        assert db_entity.token_hash == "更新后数据2"
    
    def test_deactivate_transaction_commit(self, unit_test_db: Session):
        """测试deactivate - 事务提交验证
        ...
        update_data = {"ip_address": "事务测试数据"}
        result = SessionRepository.deactivate(unit_test_db, entity, update_data)
        
        # 验证事务已提交
        unit_test_db.expire_all()
        db_entity = unit_test_db.query(Session).filter_by(id=entity.id).first()
        assert db_entity.ip_address == "事务测试数据"
    
    def test_deactivate_specialized_method(self, unit_test_db: Session):
        """测试deactivate - 专用方法测试（如有）
        ...
        # TODO: 如果有专用更新方法，在这里测试
        # 例如:
        # entity = SessionFactory.create(status='active')
        # result = SessionRepository.update_status(unit_test_db, entity.id, 'inactive')
        # assert result.status == 'inactive'
        pass
    ...
    def test_deactivate_user_sessions_query(self, unit_test_db: Session):
        ...
        # 执行Repository方法
        results = SessionRepository.deactivate_user_sessions(unit_test_db)  # TODO: 根据实际方法签名调整参数
        
        # 验证查询结果
        assert len(results) > 0

    def test_delete_expired_sessions_physical_delete(self, unit_test_db: Session):
        """测试delete_expired_sessions - 物理删除验证
        ...
        # 执行物理删除
        result = SessionRepository.delete_expired_sessions(unit_test_db, entity_id)
    ...
    def test_delete_success(self, unit_test_db: Session):
        """测试delete - 删除成功"""
        # 准备测试数据
        entity = Session(user_id=1, token_hash="待删除数据", expires_at="待删除数据", last_accessed_at="待删除数据", is_active=True)
        unit_test_db.add(entity)
        unit_test_db.commit()
        entity_id = entity.id
