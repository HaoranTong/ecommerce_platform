...
from sqlalchemy import (DECIMAL, JSON, Boolean, CheckConstraint, Column, Date,
...
class MemberLevel(Base, TimestampMixin):
    ...
    min_points = Column(
        Integer, nullable=False, default=0, comment="达到该等级所需最少积分"
    )
    discount_rate = Column(
        DECIMAL(4, 3), nullable=False, default=1.000, comment="折扣率，0.9表示9折"
    )
...
class PointTransaction(Base, TimestampMixin):
    ...
    transaction_type = Column(
        String(50), nullable=False, comment="交易类型：earn/use/expire/freeze/unfreeze"
    )
    points_change = Column(
        Integer, nullable=False, comment="积分变动数量（正数为获得，负数为消费）"
    )
    reference_id = Column(String(100), comment="关联业务ID（订单ID、活动ID等）")
    reference_type = Column(
        String(50), comment="关联业务类型（order/activity/manual等）"
    )
    ...
    status = Column(
        ...
        nullable=False,
        default="completed",
        comment="交易状态：pending/completed/cancelled",
    )
...
class MemberProfile(Base, TimestampMixin):
    ...
    # 会员标识
    member_code = Column(
        String(20), nullable=False, comment="会员编号，如M202409170001"
    )
    user_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, comment="关联用户ID"
    )

    # 等级信息
    level_id = Column(
        Integer,
        ForeignKey("member_levels.id"),
        nullable=False,
        default=1,
        comment="当前会员等级ID",
    )
    ...
    # 消费统计
    total_spent = Column(
        DECIMAL(15, 2), nullable=False, default=0.00, comment="累计消费金额"
    )

    # 会员信息
    join_date = Column(
        Date, nullable=False, default=func.current_date(), comment="入会日期"
    )
    ...
    # 状态控制
    status = Column(
        Integer, nullable=False, default=1, comment="状态: 1=正常, 2=冻结, 3=注销"
    )
...
class MemberPoint(Base, TimestampMixin):
    ...
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, comment="用户ID")
    level_id = Column(
        Integer, ForeignKey("member_levels.id"), nullable=False, comment="会员等级ID"
    )

    # 积分统计 - 严格按照原计划字段设计
    current_points = Column(Integer, nullable=False, default=0, comment="当前可用积分")
    total_earned = Column(
        Integer, nullable=False, default=0, comment="历史累计获得积分"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "current_points >= 0", name="check_current_points_non_negative"
        ),
