    ...
    def get_member_profile(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            profile_data = {
                ...
                "benefits": {
                    "free_shipping": (
                        level_info.benefits.get("free_shipping", False)
                        if level_info.benefits
                        else False
                    ),
                    "birthday_gift": (
                        level_info.benefits.get("birthday_gift", False)
                        if level_info.benefits
                        else False
                    ),
                    "priority_service": (
                        level_info.benefits.get("priority_service", False)
                        if level_info.benefits
                        else False
                    ),
                    "exclusive_events": (
                        level_info.benefits.get("exclusive_events", False)
                        if level_info.benefits
                        else False
                    ),
                    "points_multiplier": (
                        level_info.benefits.get("points_multiplier", False)
                        if level_info.benefits
                        else False
                    ),
                    "custom_service": (
                        level_info.benefits.get("custom_service", False)
                        if level_info.benefits
                        else False
                    ),
                },
...
class BenefitService:
    ...
    def calculate_discount(self, user_id: int, order_amount: Decimal) -> Decimal:
        ...
        try:
            ...
            logger.info(
                f"计算折扣: user_id={user_id}, 原价={order_amount}, 折后={discounted_amount}"
            )
...
class EventService:
    ...
    def join_event(
        self, user_id: int, event_id: int, data: dict = None
    ) -> Dict[str, Any]:
        """
        参与活动
        ...
        try:
            ...
            logger.error(
                f"参与活动失败: user_id={user_id}, event_id={event_id}, error={e}"
            )
            return {"success": False, "message": "参与活动失败"}
