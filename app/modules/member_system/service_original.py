...
class MemberService:
    ...
    def check_level_upgrade(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # ================== 获取当前等级信息 ==================
            current_level = self.db.query(MemberLevel).filter(
                MemberLevel.id == member.level_id
            ).first()
            ...
            next_levels = self.db.query(MemberLevel).filter(
                MemberLevel.id > current_level.id
            ).order_by(MemberLevel.id).all()
    ...
    def _calculate_upgrade_progress(self, member: MemberProfile) -> Optional[Dict[str, Any]]:
        """
        计算会员升级进度
        ...
        """
        # 查询下一等级信息
        next_level = self.db.query(MemberLevel).filter(
            MemberLevel.id > member.level_id
