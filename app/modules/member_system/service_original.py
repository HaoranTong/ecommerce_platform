...
# 本地应用导入
from app.modules.member_system.models import (MemberLevel, MemberPoint,
                                              MemberProfile, PointTransaction)
from app.modules.member_system.schemas import (ActivityParticipationCreate,
                                               BenefitUsageCreate,
                                               MemberCreate, MemberRead,
                                               MemberUpdate, MemberWithDetails,
                                               PointSummary,
...
class MemberService:
    ...
    def create_member(self, user_id: int, member_data: dict) -> MemberProfile:
        ...
        try:
            ...
            # 默认为注册会员（level_id=1）
            initial_level = (
                self.db.query(MemberLevel).filter(MemberLevel.id == 1).first()
            )
            ...
            # ================== 记录安全审计日志 ==================
            security_logger.log_member_creation(
                user_id,
                {
            ...
            logger.info(
                f"创建新会员成功: user_id={user_id}, member_id={new_member.member_id}"
            )
            return new_member
    ...
    def update_member_profile(self, user_id: int, update_data: dict) -> MemberProfile:
        """
        更新会员资料信息
        ...
        try:
            ...
            # ================== 记录安全审计日志 ==================
            security_logger.log_member_update(
                user_id,
                {
    ...
    def check_level_upgrade(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 注：这里暂时简化为基于等级ID排序，后续可根据积分规则完善
            next_levels = (
                self.db.query(MemberLevel)
                .filter(MemberLevel.id > current_level.id)
                .order_by(MemberLevel.id)
                .all()
            )
            ...
            logger.info(
                f"会员等级自动升级: user_id={user_id}, {old_level_name} -> {target_level.level_name}"
            )
            return upgrade_info
    ...
    def _calculate_point_summary(self, user_id: int) -> Dict[str, Any]:
        ...
        # 查询积分统计
        total_earned = (
            self.db.query(func.coalesce(func.sum(PointTransaction.points), 0))
            .filter(
                and_(
        ...
        total_used = (
            self.db.query(func.coalesce(func.sum(func.abs(PointTransaction.points)), 0))
            .filter(
                and_(
                    PointTransaction.user_id == user_id,
        ...
        return {
            ...
            "expiring_date": (
                expiry_threshold.isoformat() if expiring_points > 0 else None
            ),
        }
    ...
    def _get_member_benefits(self, level_id: int) -> Dict[str, bool]:
        ...
        """
        # 暂时返回空权益列表，等MembershipBenefit模型实现后再完善
        benefits = []
    ...
    def _calculate_upgrade_progress(
        ...
        progress_percentage = (
            min(100.0, (current_spent / required_spent) * 100)
            if required_spent > 0
            else 100.0
        )
    ...
    def _cache_member_info(self, member: MemberProfile) -> None:
        """
        缓存会员信息
        ...
        try:
            ...
            self.redis.setex(
                cache_key, self.cache_ttl, json.dumps(member_data, ensure_ascii=False)
            )

        except Exception as e:
            logger.warning(
                f"缓存会员信息失败: member_id={member.member_id}, error={str(e)}"
            )
...
class PointService:
    ...
    def earn_points(
        ...
        try:
            ...
            expiry_date = datetime.utcnow() + timedelta(
                days=365 * self.point_expiry_years
            )
            ...
            # ================== 记录安全审计日志 ==================
            security_logger.log_point_transaction(
                user_id,
                {
            ...
            logger.info(
                f"积分获取成功: user_id={user_id}, points={points}, event={event_type}"
            )
            return transaction
    ...
    def use_points(
        ...
        try:
            ...
            logger.info(
                f"积分使用成功: user_id={user_id}, points={points}, event={event_type}"
            )
            return transaction
    ...
    def _get_available_points(self, user_id: int) -> int:
        ...
        try:
            ...
            total_used = (
                self.db.query(
                    func.coalesce(func.sum(func.abs(PointTransaction.points)), 0)
                )
                .filter(
                    and_(
                        PointTransaction.user_id == user_id,
...
class BenefitService:
    ...
    def check_benefit_eligibility(
        ...
        if benefit_type not in self.benefit_types:
            raise HTTPException(
                status_code=400, detail=f"无效的权益类型: {benefit_type}"
            )
        ...
        try:
            ...
            # ================== 查询权益配置 ==================
            benefit = (
                self.db.query(MembershipBenefit)
                .filter(
                    and_(
                ...
                eligibility_info.update(
                    {
                        ...
                        "level_required": True,
                        "usage_limit": (
                            benefit.usage_limit if benefit.usage_limit else 999999
                        ),
                        "used_count": usage_stats["used_count"],
                        "remaining_count": max(
                            0,
                            (benefit.usage_limit or 999999) - usage_stats["used_count"],
                        ),
            ...
            self.redis.setex(
                cache_key,
                self.cache_ttl,
                json.dumps(eligibility_info, ensure_ascii=False),
            )
    ...
    def use_benefit(
        ...
        try:
            ...
            if not eligibility["eligible"]:
                raise HTTPException(
                    status_code=400,
                    detail=f"用户不具备{eligibility['benefit_name']}权益",
                )
            ...
            # ================== 记录安全审计日志 ==================
            security_logger.log_benefit_usage(
                user_id,
                {
            ...
            logger.info(
                f"权益使用成功: user_id={user_id}, benefit={benefit_type}, reference={reference_id}"
            )
            return usage_record
            ...
            self.db.rollback()
            logger.error(
                f"使用权益失败: user_id={user_id}, benefit_type={benefit_type}, error={str(e)}"
            )
    ...
    def get_benefit_usage_history(
        ...
        try:
            ...
            usage_records = (
                query.order_by(desc(BenefitUsage.used_at))
                .offset((page - 1) * limit)
                .limit(limit)
                .all()
            )
                ...
                usage_list.append(
                    {
                        ...
                        "benefit_name": self.benefit_types.get(
                            record.benefit_type, record.benefit_type
                        ),
                        "reference_id": record.reference_id,
                        "description": record.description,
                        "benefit_value": (
                            float(record.benefit_value)
                            if record.benefit_value
                            else None
                        ),
    ...
    def _clear_benefit_cache(self, user_id: int, benefit_type: str) -> None:
        ...
        try:
            ...
            logger.warning(
                f"清除权益缓存失败: user_id={user_id}, benefit_type={benefit_type}, error={str(e)}"
            )
...
class EventService:
    ...
    def create_activity(
        ...
        try:
            ...
            if start_time >= end_time:
                raise HTTPException(
                    status_code=400, detail="活动开始时间必须早于结束时间"
                )

            if start_time <= datetime.utcnow():
                raise HTTPException(
                    status_code=400, detail="活动开始时间必须晚于当前时间"
                )
            ...
            logger.info(
                f"创建会员活动成功: activity_id={activity.activity_id}, title={title}"
            )
            return activity
    ...
    def join_activity(self, user_id: int, activity_id: int) -> ActivityParticipation:
        ...
        try:
            ...
            activity = (
                self.db.query(MemberActivity)
                .filter(MemberActivity.activity_id == activity_id)
                .first()
            )
            if not activity:
            ...
            # ================== 记录安全审计日志 ==================
            security_logger.log_activity_participation(
                user_id,
                {
            ...
            logger.info(
                f"用户参与活动成功: user_id={user_id}, activity_id={activity_id}"
            )
            ...
            self.db.rollback()
            logger.error(
                f"用户参与活动失败: user_id={user_id}, activity_id={activity_id}, error={str(e)}"
            )
            raise HTTPException(status_code=500, detail="参与活动失败")

    def get_user_activities(
        self, user_id: int, status: Optional[str] = None, page: int = 1, limit: int = 20
    ) -> Dict[str, Any]:
        """
        获取用户参与的活动列表
        ...
        try:
            ...
            participations = (
                query.order_by(desc(ActivityParticipation.participation_time))
                .offset((page - 1) * limit)
                .limit(limit)
                .all()
            )
    ...
    def _cache_activity_info(self, activity: MemberActivity) -> None:
        ...
        try:
            ...
            self.redis.setex(
                cache_key, self.cache_ttl, json.dumps(activity_data, ensure_ascii=False)
            )

        except Exception as e:
            logger.warning(
                f"缓存活动信息失败: activity_id={activity.activity_id}, error={str(e)}"
            )
    ...
    def _clear_activity_cache(self, activity_id: int) -> None:
        ...
        try:
            ...
            logger.warning(
                f"清除活动缓存失败: activity_id={activity_id}, error={str(e)}"
            )
