...
def get_cart_service(
    db: Session = Depends(get_db), redis_client: Redis = Depends(get_redis_connection)
) -> CartService:
    """
    获取购物车服务实例
...
def validate_cart_business_rules(total_items: int, quantity: int) -> None:
    """
    验证购物车业务规则
    ...
    if not (
        CartBusinessRules.MIN_QUANTITY_PER_ITEM
        <= quantity
