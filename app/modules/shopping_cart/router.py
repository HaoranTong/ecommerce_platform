...
async def add_item_to_cart(
    ...
    **响应示例：**
    ...
    try:
        logger.info(
            f"添加商品到购物车: user_id={user_id}, sku_id={request.sku_id}, quantity={request.quantity}"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="添加商品失败"
        )
...
async def get_cart(
    ...
    **响应示例：**
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取购物车失败"
        )
...
async def update_item_quantity(
    ...
    **请求示例：**
    ...
    try:
        logger.info(
            f"更新商品数量: user_id={user_id}, item_id={item_id}, quantity={request.quantity}"
        )
        ...
        # 调用业务服务
        cart_response = await cart_service.update_quantity(
            user_id, item_id, request.quantity
        )
        ...
        logger.error(
            f"更新商品数量异常: user_id={user_id}, item_id={item_id}, error={str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="更新商品数量失败"
        )
...
async def delete_cart_item(
    ...
    **路径参数：**
    ...
    try:
        ...
        logger.error(
            f"删除购物车商品异常: user_id={user_id}, item_id={item_id}, error={str(e)}"
        )
        raise HTTPException(
...
async def batch_delete_items(
    ...
    **请求示例：**
    ...
    try:
        logger.info(
            f"批量删除购物车商品: user_id={user_id}, item_ids={request.item_ids}"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="批量删除商品失败"
        )
...
async def clear_cart(
    ...
    **注意事项：**
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="清空购物车失败"
        )
