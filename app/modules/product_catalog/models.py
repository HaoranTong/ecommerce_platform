...
from sqlalchemy import (DECIMAL, Boolean, Column, DateTime, ForeignKey, Index,
                        Integer, String, Text)
...
class Product(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        Index(
            "idx_products_brand_category", "brand_id", "category_id"
        ),  # 品牌分类复合索引
        Index(
            "idx_products_status_published", "status", "published_at"
        ),  # 状态发布时间复合索引
...
class SKU(Base, TimestampMixin):
    ...
    attributes_rel = relationship(
        "SKUAttribute", back_populates="sku", cascade="all, delete-orphan"
    )
    images_rel = relationship("ProductImage", back_populates="sku")

    # 索引
    __table_args__ = (Index("idx_skus_product_active", "product_id", "is_active"),)
