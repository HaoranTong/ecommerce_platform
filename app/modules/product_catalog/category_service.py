...
class CategoryService:
    ...
    def get_category_by_id(
        db: Session, category_id: int, include_children: bool = False
    ) -> Optional[Category]:
        """
        根据ID获取分类
    ...
    def get_category_tree(
        db: Session, parent_id: Optional[int] = None, is_active: Optional[bool] = True
    ) -> List[Dict[str, Any]]:
        """
        获取分类树结构
    ...
    def _would_create_cycle(
        db: Session, category_id: int, new_parent_id: Optional[int]
    ) -> bool:
        """
        检查移动分类是否会形成循环引用
