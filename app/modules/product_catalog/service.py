...
class ProductService:
    ...
    def create_product(
        ...
        if category_id:
            ...
            if not category:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="指定的分类不存在"
                )
        ...
        try:
            ...
            db.rollback()
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="商品创建失败，数据冲突"
            )
    ...
    def update_product(db: Session, product_id: int, **kwargs) -> Optional[Product]:
        ...
        if "sku" in kwargs and kwargs["sku"] != product.sku:
            existing = (
                db.query(Product)
                .filter(and_(Product.sku == kwargs["sku"], Product.id != product_id))
                .first()
            )
            if existing:
                raise HTTPException(
        ...
        if "category_id" in kwargs and kwargs["category_id"]:
            category = (
                db.query(Category).filter(Category.id == kwargs["category_id"]).first()
            )
            if not category:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="指定的分类不存在"
        ...
        try:
            ...
            db.rollback()
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="商品更新失败，数据冲突"
            )

    @staticmethod
    def update_stock(
        db: Session, product_id: int, quantity_change: int
    ) -> Optional[Product]:
        """
        更新商品库存
        ...
        if new_quantity < 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="库存不足，无法完成操作"
            )
    ...
    def get_low_stock_products(db: Session, threshold: int = 10) -> List[Product]:
        ...
        return (
            db.query(Product)
            .filter(
                and_(Product.stock_quantity <= threshold, Product.status == "active")
            )
            .all()
        )
