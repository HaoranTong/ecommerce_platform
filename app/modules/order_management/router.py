...
from .dependencies import (get_current_admin_user_validated,
                           get_current_authenticated_user, get_order_service,
                           validate_order_access,
                           validate_order_creation_permission,
                           validate_order_status_update_permission,
...
async def create_order(
    ...
    try:
        ...
        order = await order_service.create_order(
            order_data=order_data, user_id=current_user.id
        )
...
@router.get(
    "/order-management/orders/{order_id}", response_model=ApiResponse[OrderResponse]
)
async def get_order_detail(order: Order = Depends(validate_order_access)):
    """
    获取订单详情
...
@router.patch(
    "/order-management/orders/{order_id}/status",
    response_model=ApiResponse[OrderResponse],
)
...
@router.post(
    "/order-management/orders/{order_id}/cancel", response_model=ApiResponse[dict]
)
...
@router.get(
    "/order-management/orders/{order_id}/items",
    response_model=ApiResponse[List[OrderItemResponse]],
)
async def get_order_items(
    ...
    try:
        ...
        order_items = await order_service.get_order_items(
            order_id=order.id, user_id=None  # 依赖注入已确保用户有访问权限
        )
...
@router.get(
    "/order-management/orders/{order_id}/history",
...
@router.get(
    "/order-management/statistics", response_model=ApiResponse[OrderStatisticsResponse]
)
async def get_order_statistics(
    ...
    try:
        ...
        # 获取统计信息
        statistics = await order_service.calculate_order_statistics(
