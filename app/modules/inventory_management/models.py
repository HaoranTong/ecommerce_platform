...
class InventoryStock(BaseModel):
    ...
    available_quantity = Column(
        Integer, nullable=False, default=0, comment="可用库存数量"
    )
    reserved_quantity = Column(
        Integer, nullable=False, default=0, comment="预占库存数量"
    )

    # 预警阈值
    warning_threshold = Column(
        Integer, nullable=False, default=10, comment="库存预警阈值"
    )
    critical_threshold = Column(
        Integer, nullable=False, default=5, comment="库存严重不足阈值"
    )
    ...
    reservations = relationship(
        "InventoryReservation", back_populates="stock", cascade="all, delete-orphan"
    )
    transactions = relationship(
        "InventoryTransaction", back_populates="stock", cascade="all, delete-orphan"
    )
...
class InventoryReservation(BaseModel):
    ...
    id = Column(Integer, primary_key=True, index=True)
    sku_id = Column(
        Integer, ForeignKey("inventory_stocks.sku_id"), nullable=False, comment="SKU ID"
    )

    # 预占信息
    reservation_type = Column(
        SqlEnum(ReservationType), nullable=False, comment="预占类型"
    )
    reference_id = Column(
        String(100), nullable=False, comment="关联ID（用户ID或订单ID）"
    )
...
class InventoryTransaction(BaseModel):
    ...
    transaction_type = Column(
        SqlEnum(TransactionType), nullable=False, comment="变动类型"
    )
    quantity_change = Column(
        Integer, nullable=False, comment="变动数量（正数为增加，负数为减少）"
    )
