...
class InventoryService:
    ...
    def get_batch_inventory(self, sku_ids: List[int]) -> List[SKUInventoryRead]:
        ...
        inventories = (
            self.db.query(InventoryStock)
            .filter(
                InventoryStock.sku_id.in_(sku_ids), InventoryStock.is_active == True
            )
            .all()
        )
    ...
    def create_sku_inventory(self, inventory_data) -> Dict:
        ...
        existing = (
            self.db.query(InventoryStock)
            .filter(InventoryStock.sku_id == inventory_data.sku_id)
            .first()
        )
    ...
    async def release_reservation(self, reservation_id: str, user_id: int) -> bool:
        ...
        try:
            for reservation in reservations:
                ...
                inventory = (
                    self.db.query(InventoryStock)
                    .filter(InventoryStock.sku_id == reservation.sku_id)
                    .with_for_update()
                    .first()
                )
    ...
    async def adjust_inventory(
        ...
        inventory = (
            self.db.query(InventoryStock)
            .filter(InventoryStock.sku_id == sku_id, InventoryStock.is_active == True)
            .with_for_update()
            .first()
        )
            ...
            if hasattr(query_or_threshold, "level"):
                ...
                if query_or_threshold.level == "warning":
                    db_query = db_query.filter(
                        ...
                        < InventoryStock.warning_threshold
                    )
                elif query_or_threshold.level == "critical":
                    db_query = db_query.filter(
                        InventoryStock.available_quantity
                        < InventoryStock.critical_threshold
                    )
            else:
                # 是数值阈值
                db_query = db_query.filter(
    ...
    def check_inventory_consistency(self) -> ConsistencyCheckResponse:
        ...
        # 查询所有活跃库存
        inventories = (
            self.db.query(InventoryStock).filter(InventoryStock.is_active == True).all()
        )
    ...
    def get_transaction_logs(
        ...
        if query.operator_id:
            db_query = db_query.filter(
                InventoryTransaction.operator_id == query.operator_id
            )
