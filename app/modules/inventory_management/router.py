...
from .schemas import (  # 基础Schemas; 批量操作Schemas; 预占相关Schemas; 库存操作Schemas; 管理相关Schemas; 历史查询Schemas; 系统维护Schemas; 通用响应Schemas
    AdjustmentResponse, APIResponse, BatchInventoryQuery, CleanupResponse,
    ConsistencyCheckResponse, DeductResponse, InventoryAdjustment,
    InventoryDeductRequest, InventoryTransactionRead, LowStockItem,
    LowStockQuery, PaginatedResponse, ReleaseReservationRequest,
    ReservationResponse, ReserveRequest, SKUInventoryCreate, SKUInventoryRead,
    SKUInventorySimple, SKUInventoryUpdate, ThresholdUpdate, TransactionQuery,
...
@router.get(
    "/inventory-management/stock/{sku_id}",
...
@router.post(
    "/inventory-management/stock/batch",
    response_model=List[SKUInventorySimple],
    summary="批量获取SKU库存",
)
    ...
    try:
        ...
        background_tasks.add_task(
            ...
            "inventory.stock.reserved",
            {
                ...
                "user_id": current_user.id,
                "items": [
                    {"sku_id": item.sku_id, "quantity": item.reserved_quantity}
...
async def release_reservation(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="预占记录不存在或已过期"
            )
...
@router.delete(
    "/inventory-management/reserve/user/{user_id}", summary="批量释放用户预占"
)
async def release_user_reservations(
    ...
    if str(current_user.id) != user_id and not current_user.is_admin:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="权限不足，只能释放自己的预占"
        )
...
@router.post(
    "/inventory-management/deduct", response_model=DeductResponse, summary="库存扣减"
)
async def deduct_inventory(
    ...
    try:
        result = await service.deduct_inventory(
            order_id=request.order_id, items=request.items, operator_id=current_user.id
        )
        ...
        background_tasks.add_task(
            ...
            "inventory.stock.deducted",
            {
                ...
                "items": [
                    {"sku_id": item.sku_id, "quantity": item.deducted_quantity}
...
@router.post(
    "/inventory-management/adjust/{sku_id}",
...
@router.get(
    "/inventory-management/low-stock",
...
@router.get(
    "/inventory-management/logs/{sku_id}",
...
@router.post(
    "/inventory-management/maintenance/cleanup-reservations",
...
@router.post(
    "/inventory-management/maintenance/consistency-check",
...
async def update_sku_inventory_config(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="SKU库存记录不存在"
            )
