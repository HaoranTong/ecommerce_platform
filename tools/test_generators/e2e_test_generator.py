...
功能: 生成完整业务流程的端到端测试代码，模拟真实用户操作场景
使用方法: 通过BaseTestGenerator继承，由主生成器调用generate_e2e_tests方法
使用场景: 电商平台完整业务流程测试，如用户注册到下单的完整链路测试

生成的测试场景:
1. 用户生命周期测试 - 注册/登录/权限/注销完整流程
2. 业务流程测试 - 购物车/下单/支付/物流完整链路
3. 跨模块集成测试 - 多模块协作的复合业务场景
4. 数据一致性测试 - 跨服务数据同步和一致性验证
5. 异常恢复测试 - 业务中断后的恢复能力测试

输出位置: tests/e2e/test_{module}_e2e.py
测试框架: pytest + FastAPI TestClient + 真实数据库
