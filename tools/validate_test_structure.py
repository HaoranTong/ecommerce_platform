...
class TestStructureValidator:
    ...
    def _validate_test_function_name(self, func_name, file_path):
        ...
        has_result_keyword = any(
            keyword in func_name.lower() for keyword in result_keywords
        )
    ...
    def validate_required_imports(self, file_path):
        ...
        for required in required_imports:
            if (
                f"import {required}" not in content
                and f"from {required}" not in content
            ):
                self.errors.append(f"❌ {file_path}: 缺少必需导入 '{required}'")

        # 检查是否使用了禁止的导入
        forbidden_patterns = ["from unittest.mock import", "import unittest.mock"]

        for pattern in forbidden_patterns:
            if pattern in content:
                self.errors.append(
                    f"❌ {file_path}: 禁止使用 '{pattern}'，请使用 pytest-mock"
                )
    ...
    def validate_test_structure(self, file_path):
        ...
        if "setup_method" in content or "teardown_method" in content:
            if "setup_method" in content and "teardown_method" not in content:
                self.warnings.append(
                    f"⚠️ {file_path}: 有setup_method但缺少teardown_method"
                )
    ...
    def run_validation(self, test_dir="tests/"):
        ...
        for root, dirs, files in os.walk(test_dir):
            ...
            for file in files:
                if file.endswith(".py") and (
                    file.startswith("test_") or file == "conftest.py"
                ):
        ...
        print(f"\n📋 验证摘要:")
        print(
            f"   ✅ 通过: {test_files_found - len(set(e.split(':')[1] for e in self.errors if ':' in e))} 个文件"
        )
