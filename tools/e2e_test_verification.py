...
class E2ETestVerification:
    ...
    def _stage2_quality_validation(self) -> Dict[str, Any]:
        ...
        validation_files = list(
            (self.project_root / "docs" / "analysis").glob(
        ...
        if validation_files:
            ...
            try:
                ...
                basic_checks_pass = (
                    "语法检查: 4/4 通过" in report_content
                    and "导入验证: 4/4 通过" in report_content
                )
            ...
            print("  ❌ 未找到验证报告")
            return {"success": False, "error": "no_validation_report"}
...
'''
        ...
        for file_path in test_files:
            ...
            try:
                ...
                try:
                    ...
                    results["details"][file_path] = {"syntax": "pass", "import": "pass"}
                    print(f"    ✅ {file_path}")
    ...
    def _test_pytest_collection(self) -> Dict[str, Any]:
        ...
        try:
            # 仅测试工厂文件，避免复杂依赖
            factory_file = (
                self.project_root / f"tests/factories/{self.test_module}_factories.py"
            )

            if not factory_file.exists():
                return {"success": False, "error": "factory_file_not_found"}
    ...
    def _stage5_result_reporting(self) -> Dict[str, Any]:
        ...
        try:
            ...
            report_file = (
                ...
                / "docs"
                / "analysis"
                / f"e2e_verification_report_{timestamp}.md"
            )
...
"""
        ...
        # 添加各阶段结果
        for stage_name, stage_result in self.verification_results.get(
            "stages", {}
        ).items():
...
"""
    ...
    def _evaluate_overall_success(self):
        ...
        critical_success = all(
            stages.get(stage, {}).get("success", False) for stage in critical_stages
        )
        ...
        # 验证结果评估
        validation_acceptable = stages.get("validation", {}).get(
            "passed_validation", False
        )

        # 整体判断：关键阶段成功 + 验证可接受 + 部分执行成功
        self.verification_results["overall_success"] = (
            critical_success and validation_acceptable and execution_partial
        )

    def _finalize_results(self, start_time: datetime):
        """完成验证并计算耗时"""
        end_time = datetime.now()
        self.verification_results["execution_time"] = (
            end_time - start_time
        ).total_seconds()
