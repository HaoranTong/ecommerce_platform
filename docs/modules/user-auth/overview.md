...
### 主要职责
用户认证与权限管理的核心模块，提供完整的身份验证和授权服务：
- 用户注册登录管理 - 支持手机号、邮箱、微信多种登录方式
- JWT身份认证 - 基于Token的安全认证机制
- 权限控制管理 - 基于角色的权限访问控制(RBAC)
- 用户信息管理 - 用户资料、实名认证、安全设置

### 业务价值
- **核心价值**: 为平台提供安全可靠的身份认证和权限管理基础设施
- **用户收益**: 简化注册登录流程，保障账户安全，提供个性化服务
- **系统收益**: 统一身份管理，细粒度权限控制，安全审计跟踪

### 模块边界
- **包含功能**: 用户注册登录、JWT认证、权限管理、用户信息管理、安全设置
- **排除功能**: 支付信息、订单数据、商品管理、第三方业务数据
- **依赖模块**: database-core(数据库)、redis-cache(缓存)
- **被依赖**: 所有业务模块(商品、订单、购物车等)都依赖用户认证
