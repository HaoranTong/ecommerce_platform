...
### 目录结构
```
app/modules/supplier_management/
├── __init__.py
├── router.py           # ✅ API路由实现
├── service.py          # ✅ 业务逻辑实现
├── repository.py       # 🔄 数据访问层
...
@router.post("/supplier-management/{resource}", response_model={Schema}Response)
...
```python
import pytest
from app.modules.supplier_management.service import {Module}Service
...
### 依赖注入配置
```python
# app/modules/supplier_management/dependencies.py
