...
### 模块文件命名
- 路由文件：`router.py` (模块内统一命名)
- 数据模型：`models.py` (模块内数据模型定义)
- Pydantic模式：`schemas.py` (模块内请求/响应模型)
- 服务类：`service.py` (模块内业务逻辑)
- 依赖注入：`dependencies.py` (模块内依赖管理)

### 模块化架构规范
- **core/** - 核心基础设施：数据库、缓存、认证等跨模块组件
- **shared/** - 共享组件：仅限技术强制性组件（如ORM基础类）和跨模块共享的数据模型
- **adapters/** - 第三方适配器：封装外部服务集成，提供统一接口
- **modules/** - 业务模块：按业务域组织，每个模块包含完整的垂直切片，**API schemas严格模块内管理**

### 模块独立性原则
- **schemas.py** - 严格限制为模块内组件，各模块独立定义API请求/响应模型
- **禁止跨模块共享API相关组件** - 避免增加模块间耦合，保障未来微服务拆分能力
- **通过文档标准保证一致性** - 而非代码共享，使用模板和工具避免重复
...
"""
文件名：router.py
文件路径：app/modules/user_auth/router.py
功能描述：用户认证模块的API路由定义
主要功能：
- 用户注册、登录、信息更新
- 用户权限验证和管理
...
- app.modules.user_auth.schemas.UserCreate, UserRead: 用户Schema
- app.modules.user_auth.service.UserService: 用户业务逻辑
