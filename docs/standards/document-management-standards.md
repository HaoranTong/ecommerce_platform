...
#### planning/目录详细标准 
**职责定位**: 项目进度管理和开发规划，面向AI开发的实用项目管理
**文档特征**: 结构化项目计划、可执行的开发路线图、AI友好的进度跟踪
**核心原则**: 实用导向、AI可读、单人项目优化、避免团队管理复杂度

**必需文档结构**:
```
planning/
├── README.md                    # 项目管理层导航和边界定义
├── DEVELOPMENT_PLAN.md          # 开发计划和里程碑管理
├── RELEASE_PLAN.md              # 发布计划和版本管理 (可选)
└── RISK_REGISTER.md             # 风险识别和应对措施 (可选)
```

**DEVELOPMENT_PLAN.md标准**:
- **必须包含**: 当前阶段、下阶段计划、优先级定义、完成标准
- **AI优化要求**: 使用表格和清单格式，明确状态标识(✅🔄📝❌)
- **结构要求**: 概览→当前状态→下步计划→优先级→风险提示
- **禁止内容**: 团队分工、人员管理、预算管理、会议计划
- **更新频率**: 每完成重要功能或遇到计划调整时更新

**README.md标准**:
- **必须包含**: 层级定位、职责边界、文档导航、更新规范
- **边界要求**: 严格区分与requirements/、design/、operations/的职责边界
- **AI优化**: 使用emoji标识、表格化信息、快速导航结构
- **禁止内容**: 具体开发计划细节、技术实现方案、业务需求分析

**内容边界约束**:
- ✅ **应该包含**: 开发阶段规划、功能优先级、完成标准、风险识别、发布计划
- ❌ **禁止包含**: 技术架构设计、具体API规范、数据库设计、业务需求细节
- ⚠️ **边界说明**: 基于requirements/输入制定计划，为design/提供开发优先级指导
...
| **api-implementation.md** | modules/ | API实施记录 | ✅ 完整标准 | API实现，禁止规范定义 |
...
**测试文档管理职责分工**：
- **测试用例代码** → `tests/` 目录，遵循 [testing-standards.md](testing-standards.md) 管理
- **测试设计策略** → `docs/design/modules/*/implementation.md` 测试实施章节（可选）
- **测试结果分析** → `docs/analysis/` 目录，本标准管理
- **测试工具配置** → `docs/tools/` 目录，本标准管理

**详细设计测试要求**：
- ✅ **应该包含**：测试策略选择、测试覆盖范围规划、测试环境要求
- ❌ **禁止包含**：具体测试用例代码、详细测试脚本、测试数据内容
- ⚠️ **边界说明**：设计层面的测试考虑，不替代tests/目录的具体实现
