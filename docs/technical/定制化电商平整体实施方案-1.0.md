...
- **架构持续演进原则**（2025-09-09 新增）：
  - **架构前瞻性**：从mini-MVP开始就考虑最终产品升级迭代需求，不采取临时方案
  - **可扩展设计**：所有模块设计支持功能扩展，避免后续大量重构
  - **接口稳定性**：API设计考虑向后兼容，支持版本演进
  - **数据模型前瞻**：数据库设计预留扩展字段，支持业务增长

- **开发流程原则**：
  - **详细规划先行**：每个功能模块开发前必须制定详细的功能模块开发计划
  - **需求细化**：在总需求基础上，细化该功能模块的详细设计
  - **严格执行**：细化需求 → 开发 → 单元测试 → 集成测试 → 验收
  - **测试驱动**：开发前先编写测试用例，确保功能可验收

- 核心技术原则：
  - Contract‑first（契约优先）：接口（OpenAPI）、事件 Schema、DB DDL、状态机在开发前冻结并存版本库。
  - Adapter 抽象：第三方整合（Payment/Trace/AI/Storage）必须通过 Adapter 接口实现。
  - Feature Flag：每个功能支持独立启用/禁用，支持灰度发布。
  - 底座优先但分层交付：先交付可复用底座（Event Bus / Auth / CI），并并行推进业务功能。
  - 单人+AI协作工作流：你负责业务/验收/测试；我负责代码、测试脚本、部署说明。每个交付含可运行示例与本地运行步骤。

二、总体里程碑与阶段（高层）

**当前状态**: Sprint0 已完成，正在进行第一期开发

- ✅ 准备期 Sprint0（已完成） — 基础架构、数据模型、核心API已就绪
- 🔄 第一期 Mini-MVP（当前阶段，预计4-6周） — PC后台管理 + 支付系统 + 移动端基础页面
- 第二期 完整交易闭环（6-8 周） — 小程序完整功能 + 真实支付对接 + 运营工具
- 第三期 分销/售后/对账（8–12 周）
- 第四期 AI/推荐/向量（8–12 周）
- 第五期 溯源 IoT + 链上证明 + 智能拆单/支付中台（8–12 周）
